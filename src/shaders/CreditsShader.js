const CreditsShader = {
    ...
    init: function () {
        ...
        this.uniforms.mUniform = gl.getUniformLocation(
            this.program,
            "u_model_matrix"
        );
        this.uniforms.vUniform = gl.getUniformLocation(
            this.program,
            "u_view_matrix"
        );
        this.uniforms.pUniform = gl.getUniformLocation(
            this.program,
            "u_projection_matrix"
        );
        ...
        this.uniforms.lightPositionUniform = gl.getUniformLocation(
            this.program,
            "u_light_position"
        );
        this.uniforms.lightTargetUniform = gl.getUniformLocation(
            this.program,
            "u_light_target"
        );
        this.uniforms.lightCutoffUniform = gl.getUniformLocation(
            this.program,
            "u_light_cutoff"
        );
        this.uniforms.lightOuterCutoffUniform = gl.getUniformLocation(
            this.program,
            "u_light_outer_cutoff"
        );
        this.uniforms.lightConstantUniform = gl.getUniformLocation(
            this.program,
            "u_light_constant"
        );
        this.uniforms.lightLinearUniform = gl.getUniformLocation(
            this.program,
            "u_light_linear"
        );
        this.uniforms.lightQuadraticUniform = gl.getUniformLocation(
            this.program,
            "u_light_quadratic"
        );

        this.uniforms.alphaUniform = gl.getUniformLocation(this.program, "u_alpha");
        this.uniforms.textureSamplerUniform = gl.getUniformLocation(
            this.program,
            "u_texture_sampler"
        );
        this.uniforms.kShininessUniform = gl.getUniformLocation(
            this.program,
            "u_materialShininess"
        );
